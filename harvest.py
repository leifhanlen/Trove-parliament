...
def clean_metadata(version):
    ...
    #print('creators: '+[c.strip().encode('utf-8') for c in creators])
    ...
    if isinstance(subjects, str):
        subjects = [subjects]
    record['subjects'] = [str(s).strip().encode('utf-8') for s in subjects]
...
def save_csv():
    ...
    with open('results-{}.csv'.format(datetime.now().strftime('%Y%m%d')), 'w') as csv_file:
        # The fields we're going to save
        fieldnames = ['_id', 'title', 'date', 'creators', 'source', 'subjects', 'source_url', 'trove_url']
        ...
        for version in versions:
            ...
            try:
                ...
                record['creators'] = '; '.join(str(e) for e in record['creators'])
                record['subjects'] = '; '.join(str(e) for e in record['subjects'])
...
def save_texts():
    ...
    for version in db.versions.find().batch_size(50):
        ...
        creator_name = slugify('-'.join(str(e) for e in record['creators']))
        ...
        if not os.path.exists('texts/{}'.format(filename)):
            ...
            if content:
                # Open file
                print ('Saving file...')
                with open('texts/{}'.format(filename), 'w') as text_file:
